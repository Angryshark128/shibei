...
## [2026-09-13] 分页提前停止前提不成立，改为收敛 V2EX pages_per_node

**背景**  
为压低抓取耗时，方案是「列表按时间倒序 → 翻到整页都比 `since` 老就停止分页」。实测三个分页来源，前提均不成立：
- **V2EX**：`/api/topics/show.json?node_name=X&p=N` 的 `p`（以及 `page`）**完全无效**——programmer / python / share / create 四个节点在 p=1/2/3/4 返回**逐 id 相同**的 10 条（HTTP 200）。故 `pages_per_node: 6` 每节点 6 次请求只拿到 10 帖，5 次是纯重复；单节点实测 6 页 26.4s vs 1 页 0.11s，结果同为 10 帖。
- **Dev.to**：`/api/articles?tag=X&page=N` 页内 `created_at` 乱序，跨页时间区间也重叠（p1 min < p2 max），不能据时间停止。`/api/articles/latest` 跨页区间单调递减、页内仍非严格倒序——即便可用也需换端点，属行为变更。
- **Lobste.rs**：本机到 lobste.rs 网络不可达（`Errno 101`），无法实测；配置里又混用 `hottest`（按分数排序）与按标签分页，语义不一致。

**决策**  
- 不做「按时间提前停止分页」的通用机制——前提不成立，做了就是静默丢数据。
- `config.json` 的 v2ex `pages_per_node` 由 6 改为 1：请求数 96 → 16（16 节点），抓取内容不变。

**理由**  
`p` 无效意味着第 2~6 页是纯重复请求，只消耗目标站带宽与自身时间，与宪法「增量与幂等」相悖（其违反后果正是「重复请求 API 造成限流」）。改 1 顺带修好一个隐藏问题：缓存命中条件是 `len(cached) >= pages * TOPICS_PER_PAGE`（6 × 5 = 30），而 V2EX 每节点只有 10 帖，该条件**永远不可能满足**，V2EX 的今日缓存此前从未命中过；改 1 后 `10 >= 5`，同日重跑 0 请求（实测第 1 次 1 个、第 2 次 0 个）。

**备选方案**  
- 实现「整页比 `since` 老就停止分页」— 否，候选来源全不满足时间倒序前提，会静默丢帖。
- 实现「某页无新 id 就停止」的启发式通用保护 — 否，列表在两次请求之间可能重排（Dev.to 非稳定序），存在提前截断风险；当前唯一触发者是 V2EX，配置层修掉即可，不必给全体来源换语义。
- 改用 V2EX API v2 提高覆盖率 — 暂不，v2 需用户 Personal Access Token（新增凭据面），且这是「每节点只有最新 10 帖」的覆盖率问题，与本次「请求量」目标不同，另行评估。
