...
> **面向独立开发者的社区情报工具**：从社区抓取帖子，调用 LLM 提炼产品创意、用户痛点与潜在机会，输出带可点击来源链接的 Markdown 报告，帮你低成本发现「值得做的东西」。
...
**定位**：服务独立开发者——四类洞察直指「做什么、做给谁、值不值得做」：创意点子、用户痛点、低门槛机会、技术趋势。
...
**依赖环境**：运行时只需 Python 3.10+；网络需可达来源社区 API 与用户自带的 LLM API；LLM 需配置 `OPENAI_API_KEY`（必填）及 `OPENAI_BASE_URL` / `ANALYZE_MODEL`（env 或 config 二选一）。
...
```
分析以下社区帖子，按下面每个类别分别提炼信息（同一批帖子只发一次，请一次全部输出）。
...
`analyze(topics, incremental=False)` → `{类别标题: 分组整理后的文本}`：
